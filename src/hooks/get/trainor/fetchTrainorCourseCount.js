...
/**
 * Fetches total courses and new courses within a given time range for a specific trainer.
 ...
 */
const fetchCourseCountComparison = async (timeRange, trainerId) => {
  try {
    if (!trainerId) throw new Error("Trainer ID is required");

    const coursesRef = collection(db, "Course");

    // ✅ Get total courses by this trainer (all-time count)
    const allCoursesQuery = query(coursesRef, where("UID", "==", trainerId));
    ...
    // ✅ Query courses by this trainer *within the selected time range*
    const timeRangeQuery = query(
      coursesRef,
      where("UID", "==", trainerId), // Filter by Trainer ID
      where("CreatedAt", ">=", pastTimestamp) // Filter by time range
    );
