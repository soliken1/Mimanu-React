...
const ForgotPasswordScreen = () => {
  ...
  const handlePasswordReset = async (e) => {
    ...
    if (!emailFormat.test(email)) {
      setError("The email address format is invalid. Please enter a valid email.");
  ...
  return (
    <div className="flex h-screen w-screen flex-row overflow-hidden">
      ...
      <div className="z-10 shadow-x flex h-full w-full flex-col items-center justify-center rounded-s-xl md:w-5/12">
        ...
        <form onSubmit={handlePasswordReset} className="mt-5 flex w-full flex-col gap-5 px-16">
          <div className="inner-shadow relative flex flex-row items-center rounded-xl px-8 py-2 transition-all duration-300 focus:outline-none focus:border-0">
            ...
            <input
              ...
              type="email"
            />
            <span asp-validation-for="Email" className="absolute end-0 right-6 text-xs text-red-500"></span>
          </div>
          {message && <p className="text-green-500 text-sm text-center">{message}</p>}
          {error && <p className="text-red-500 text-sm text-center">{error}</p>}
          <div className="mt-5 flex h-auto w-full items-center justify-center">
            <button className="h-12 w-64 rounded-xl bg-[#DC3A3A] text-white duration-300 hover:bg-[#BC3232]" type="submit">
              Send
            </button>
          </div>
